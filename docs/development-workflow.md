...
正式IssueにはIssue Formの構造化YAMLで要件ID、種別、説明、受入条件、必須性を記録します。さらに、デプロイ・環境構成の12回答を`deployment_answers`へ記録します。いずれも記載しただけでは承認済みになりません。

## 7. デプロイ先と環境

利用場所、利用者、利用時間、停止許容、復旧・データ損失、検証環境、本番相当データ、費用、運用担当を確認します。サービス名の選択は専門AIが条件から提案します。

人間は`ai-dev issue-approval-template --issue <番号>`で現在のIssue本文に対応する二つのコメント雛形を表示し、内容を確認してGitHub Issueへ手動投稿します。要件と環境構成の各ダイジェストが一致し、最後の判断が承認であり、`ai:approved`ラベルがある場合だけ開発開始条件を満たします。Issue本文を変えた場合はダイジェストが変わるため再承認が必要です。
...
GitHub Actionsの`AI開発オーケストレーター`をmainから手動実行し、承認済みIssue番号だけを入力します。Workflowはブランチ作成前に`issue-preflight`を実行し、非公開リポジトリ、許可actor、main上のWorkflow定義、Issue番号、要件承認、環境構成承認、`ai:approved`を照合します。MVPでは同じIssueの変更タスクは同時に1件です。

## 12–14. 設計・実装・テスト

設計開発AIが要件を仕様化し、設計、実装、単体・結合相当の自動テスト、要件追跡を更新します。ホスト検証PASS後だけIssueブランチへcommit/pushしてPRを作成し、開発Workflowはそこで停止します。mainへのpushや本番操作は行いません。

## 15. システムレビュー

通常CIが成功した後、人間がmainから手動実行する独立した`ai-quality-gates.yml`の読み取り専用AIが、設計整合、コード、セキュリティ、性能、可用性、保守性、テスト、Secret、環境分離を評価します。入力した承認済みIssueとPRの関連、許可actor、同一repository、非fork、許可branch、対象SHAを有料処理より前に検証します。PRの作成・更新だけでは有料レビューを起動しません。開発用手動Workflow内ではレビューせず、不合格時は修正受入条件を伴って差戻します。
